...
void putBacktoList(char listName[], char productName[], char productCantitate[], char productPret[])
{
    ...
    if(pointer == NULL)
    {
        ...
        while(fgets(data, 100, pointer) && flag)
        {
            ...
            if(strcmp(productName, oldName) == 0) // Cand gasim un nume identic
            {
                corect = count;
                ...
                //sprintf(productPret, "%.2f", oldPret);
                strcpy(productPret, oldPret);// Se pune pretul vechi(adica pretul pe produs)
                flag = 0;
